...
1. **Create agent module**: `src/agents/<agent_name>.py`
2. **Define agent class** inheriting from base (if any)
3. **Implement required methods**: `__init__`, `run`, `should_continue`
4. **Add docstrings** with examples
5. **Register in graph**: Update `src/graph/builder.py`
6. **Add tests**: `tests/agents/test_<agent_name>.py`
7. **Update documentation**: `docs/architecture.md`

```python
# src/agents/documenter.py
"""Documenter agent for writing documentation."""

from typing import Dict, Any
from langgraph.graph import MessagesState
from langgraphx.agents.base import BaseAgent

class DocumenterAgent(BaseAgent):
    """Agent responsible for writing and maintaining documentation.
    
    This agent generates documentation based on code changes,
    updates README files, and ensures docs stay synchronized.
    """
    ...
    def __init__(self, llm_client):
        ...
        """
        super().__init__(name="documenter", role="documentation")
        self.llm_client = llm_client
    
    def run(self, state: MessagesState) -> Dict[str, Any]:
...
1. **Create tool file**: `src/tools/<category>_tools.py`
2. **Use `@tool` decorator**
3. **Add type hints and docstring**
4. **Accept `config: RunnableConfig`** if project-scoped
5. **Validate inputs** and handle errors
6. **Add unit tests**
7. **Register in tool loader**
...
def git_status(config: RunnableConfig) -> str:
    ...
    Returns the current git status including:
    - Branch name
    - Staged changes
    - Unstaged changes
    - Untracked files
    ...
    Args:
        config: Runtime config with project_path in configurable
        
    Returns:
        Git status output as string
        
    Raises:
        GitError: If project is not a git repository
    """
...
1. **Create config directory**: `projects/<project_name>/`
2. **Add `config.yaml`** with project details:
   ```yaml
   name: myproject
   type: rust  # or elixir, python, etc.
   ...
   tools:
     ...
     test: cargo test
   ```
3. **Add custom prompts** (optional): `projects/<project_name>/prompts/`
4. **Register in main config** or let auto-discovery find it
